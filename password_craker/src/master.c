...
void usage(){
	...
	printf("mpirun -np 1 ./master [PathToFileContainsAlphabet] [num_procs] [num_slaveThreadsPerProc] [maxLengthToTest] [passwordToTest]\n");
}
...
int main(int argc, char** argv){
	...
	while(!stopComm){ 
		...
		if(flag){
			...
			case TAG_CONFIRM_EXIT:
				MPI_Recv(0,0,MPI_INT, status.MPI_SOURCE, TAG_CONFIRM_EXIT, inter, &status);
				++num_slaves_exit;
				if(num_slaves_exit == num_procs-1)
					stopComm = 1;
				break;
	...
	/* communication terminated, check if pwd is found */
	if(isFound){
		...
		printf("\n********************************************************************** \n");
		printf("<<< \x1b[32m SUCCESS \x1b[0m>>> Password is [%s] Used %lf(s)\n", str_pw, time);
		printf("********************************************************************** \n");
		free(str_pw);
	}
	else{
		printf("\n********************************************************************** \n");
		printf("<<< \x1b[31m FAILED\x1b[0m >>> All slaves don't find the password! Program quit!!!\n");
		printf("********************************************************************** \n");
	}  
