...
int main(int argc, char** argv){
			...
			//while(!flag_pw){
				...
				if(flag){
					if(moreInterval){
						...
						MPI_Isend(&moreInterval, 1, MPI_INT, status.MPI_SOURCE, TAG_NO_MORE_INTERVAL, MPI_COMM_WORLD, &req);
						//MPI_Abort(MPI_COMM_WORLD, 2); 
					}
				}
			//}
	...
	if(rank != 0){ // slave
		...
		while(!isDone){   /* Main loop */
			...
			isDone = 1;
			break; // don't request for interval any more
