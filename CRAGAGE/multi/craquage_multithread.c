...
int main(int argc, char** argv){
  ...
  //int pas=1000;
  printf("la taille de l'intervalle distribué est %d\n", pas);
  pvm_catchout(stderr);
  int cc = pvm_spawn("/home/pg305-03/TPvendredi/CRAGAGE/multi/esclave", (char**)0, 0, "", nlr_pros, filstids);
  assert(cc == nlr_pros);
  ...
  for(i=0; i<nlr_pros; i++)
    {
      ...
      //envoyer le nombre de thread
      send_num(filstids[i],7, nb_thread);
    }
